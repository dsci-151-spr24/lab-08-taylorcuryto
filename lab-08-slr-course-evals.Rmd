...
evals %>%
  summarize(means = mean(score), medians = median(score), mins =  min(score), maxs = max(score), sds = sd(score))

```
    
***This distribution is skewed to the left. This means that on average, students tend to give a higher score evaluation because when students don't tend to have much of an opinion on the class, they end up giving a higher score on average. Since the graph is clearly skewed to the left, that is what showed this observation.***
...
***There is a slight increase in slope based on the beauty average vs. the professor rating. It is a very weak correlation.***
...
***Jitter seems to be more precise. It is less aesthetically pleasing and the first scatter plot can be misleading because it is not the exact values- it just rounds them to make the graph look prettier.***
...
$$\widehat{score_i} = 4.28 - 0.12\times TenureTrack_i - 0.14\times Tenured_i$$

***The average rating for a teaching track professor is 4.38. The average rating for a tenure track professor is 0.12 points lower than a teaching track professor. The average rating for a tenured professor is 0.14 points lower than a teaching professor.***
...
$$\widehat{score_i} = 4.14 - 0.14\times teaching_i + 0.02\times Tenure Track_i$$
...
```{r}
evals <- evals %>%
  mutate(tenure_eligible = ifelse(rank %in% c("tenure track", "tenured"), "yes", "no"))
